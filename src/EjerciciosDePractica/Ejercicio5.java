//El dueño de una empresa desea planificar las decisiones financieras que tomara en el siguiente
//año. La manera de planificarlas depende de lo siguiente:
//Si actualmente su capital se encuentra con saldo negativo, pedirá un préstamo bancario para
//que su nuevo saldo sea de U$10.000. Si su capital tiene actualmente un saldo positivo pedirá un
//préstamo bancario para tener un nuevo saldo de U$20.000, pero si su capital tiene actualmente
//un saldo superior a los U$20.000 no pedirá ningún préstamo.
//Posteriormente repartirá su presupuesto de la siguiente manera:
//• U$5.000 para equipo de computo
//• U$2.000 para mobiliario
//• El resto la mitad será para la compra de insumos y la otra para otorgar incentivos al personal.
//Desplegar que cantidades se destinaran para la compra de insumos e incentivos al personal y,
//en caso de que fuera necesario, a cuánto ascendería la cantidad que se pediría al banco.
package EjerciciosDePractica;
import java.util.Scanner;
/**
 *
 * @author u20232217593
 */
public class Ejercicio5 {
    public static void main(String[] args) {
        Scanner desiciones = new Scanner(System.in);
        System.out.println("Digite su capital");
        double saldo = desiciones.nextDouble();
        String pres1= "Si";
        String pres2= "No";
        double pre;
        double insu;
        double cantidad;
        if (saldo<10000){
            Scanner desi=new Scanner(System.in);
            System.out.println("Actualmente su saldo es negativo. ¿Desea solicitar un prestamo al banco?");
            String prestamo=desi.nextLine();
            if (prestamo.equals(pres1)){
                System.out.println("Con este prestamo has adquirido un capital de 10000");
                cantidad=10000-saldo;
                System.out.println("La cantidad adquirida fue de: " + cantidad);
                pre=10000-7000;
                insu=pre/2;
                System.out.println("Para los insumos e incentivos al personal se otorgara: " + insu);    
            }
            else {
                System.out.println("Tu saldo seguira siendo de " + saldo + ".");
            }
        }
        else if (saldo>=10000 && saldo<=20000){
            Scanner desi=new Scanner(System.in);
            System.out.println("Actualmente su saldo es positivo. ¿Desea igualmente solicitar un prestamo al banco?");
            String prestamo=desi.nextLine();
               if (prestamo.equals(pres1)){
                System.out.println("Con este prestamo has adquirido un capital de 20000");
                cantidad=20000-saldo;
                System.out.println("La cantidad adquirida fue de: " + cantidad);
                pre=20000-7000;
                insu=pre/2;
                System.out.println("Para los insumos e incentivos al personal se otorgara: " + insu);   
        }
               else {
                    System.out.println("Tu saldo seguira siendo de " + saldo + ".");
               }
        }
        else if (saldo>20000){
            System.out.println("Actualmente su saldo es superior a 20000");
                pre=saldo-7000;
                insu=pre/2;
                System.out.println("Para los insumos e incentivos al personal se otorgara: " + insu);   
        }
               
        }
    
}

